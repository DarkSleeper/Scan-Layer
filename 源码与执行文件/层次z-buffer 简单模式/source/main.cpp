...
int main(int argc, char* argv[]) {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		auto test_triangle = [&](Triangle t)->bool {
			std::vector<glm::vec3> t_verts;
			t_verts.push_back(t.a);
			t_verts.push_back(t.b);
			t_verts.push_back(t.c);
			float near_z = scale_z;
			glm::vec2 rec_max(0, 0), rec_min(SCR_WIDTH, SCR_HEIGHT);
			for (int i = 0; i < 3; i++) {
				if (!in_bound(t_verts[i])) return true;
				t_verts[i].x = (t_verts[i].x + 1) / 2.0f * SCR_WIDTH;
				t_verts[i].y = (t_verts[i].y + 1) / 2.0f * SCR_HEIGHT;
				t_verts[i].z = (t_verts[i].z + 1) / 2.0f * scale_z;
				if (t_verts[i].z < near_z) {
					near_z = t_verts[i].z;
				}
				rec_min.x = fminf(rec_min.x, t_verts[i].x);
				rec_min.y = fminf(rec_min.y, t_verts[i].y);
				rec_max.x = fmaxf(rec_max.x, t_verts[i].x);
				rec_max.y = fmaxf(rec_max.y, t_verts[i].y);
			}
		...
		};
		{
			...
			for (int i = 0; i < triangle_num; i++) {
				...
				Triangle t;
				t.a = screen_vertices[idx1];
				t.b = screen_vertices[idx2];
				t.c = screen_vertices[idx3];
				if (!test_triangle(t)) {
					clip_num++;
					continue;
				}
